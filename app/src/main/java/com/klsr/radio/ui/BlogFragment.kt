...
class BlogFragment : Fragment(R.layout.fragment_blog) {
            ...
            try {
                val posts = withContext(Dispatchers.IO) {
                    try {
                        ...
                        Log.e("BlogFragment", "Hero load failed", e)
                        emptyList()
                    }
                }
                ...
                Log.e("BlogFragment", "Hero error", e)
            }
        }
    }
    ...
    private fun setupHeroSlider() {
        ...
        val adapter = BlogHeroAdapter(heroPosts) { postId ->
            if (!isAdded) return@BlogHeroAdapter
            findNavController().navigate(R.id.action_blog_to_singlePost, Bundle().apply { putInt("postId", postId) })
        }
            ...
            try {
                val result = withContext(Dispatchers.IO) {
                    try {
                        ...
                        Log.e("BlogFragment", "Posts load failed", e)
                ...
                binding.blogRecyclerView.adapter = BlogPostAdapter(allPosts) { postId ->
                    if (!isAdded) return@BlogPostAdapter
                    findNavController().navigate(R.id.action_blog_to_singlePost, Bundle().apply { putInt("postId", postId) })
                }
                ...
                Log.e("BlogFragment", "Blog error", e)
            }
        }
    }

    private fun filter(query: String) {
        val filtered = if (query.isBlank()) allPosts else allPosts.filter {
